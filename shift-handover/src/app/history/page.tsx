...
import { History, Search, Eye, Filter, Download } from "lucide-react";
import {
  cn,
  getShiftLabel,
  getStatusLabel,
...
export default function HistoryPage() {
  ...
  const handleDownload = async () => {
    ...
    const res = await fetch(`/api/history/download?${params.toString()}`, { cache: "no-store" });
    if (res.ok) {
      ...
      a.download = match ? match[1] : "handover.xlsx";
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            title={!startDate && !endDate ? "Select a date range to download" : "Download as Excel"}
